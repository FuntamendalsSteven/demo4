...
```{r}
allcontent <- merge(maternalmortality, infantmortality, by = "name", all = TRUE) # merge maternalmortality and infantmortality (all = TRUE means outer join)
...
```{r}
allcontent <- merge(allcontent, lifeexpectancy, by = "name", all = TRUE) # merge allcontent and lifeexpectancy (all - TRUE means outer join)
str(allcontent) # view the structure of the data frame post merge
```

We see that there is a region.y column and a region column, so we check for differences, and remove the redundant column.
```{r}
...
Let's aggregate the data by region as assigned. This means simplifying and summarizing the data. A suitable function is the mean. Let's apply the mean to all columns except the name column aggregated by region.
```{r}
...
Let's save the result into its own csv file.
```{r}
write.csv(regionaverage[,], "regionaverage.csv", row.names = FALSE) # saves regionaverage[,] (regionaverage data frame, all rows and columns) to csv file named "regionaverage.csv" without row.names (X column numbered 1 - 10)
```

We can verify whether the file saved correctly.
```{r}
data <- read.csv("regionaverage.csv") # reads csv file "regionaverage.csv" to dataframe object called data
str(data) # views internal structure of data
```

# Reshaping

Let's pivot the data frame we obtained as a result of the merge from wide to long.
```{r}
library(tidyr) # package needed for pivoting
cpivot <- c("maternaldeathsper100000livebirths", "infantdeathsper1000livebirths", "years") # create vector of columns to pivot, cannot pivot if columns are not same type so name and region are excluded
allcontentlong <- tidyr::pivot_longer(data = allcontent, cols = cpivot, names_to = "variable", values_to = "value", values_drop_na = TRUE) # pivots allcontent data frame, affecting columns in cpivot, changing column names to variable and adding value column. values_drop_na = TRUE to exclude NA values
allcontentlong # view resulting data frame
```

Let's save the result.
```{r}
write.csv(allcontentlong[,], "allcontentlong.csv", row.names = FALSE) # saves allcontentlong[,] (allcontentlong data frame, all rows and columns) to csv file named "allcontentlong.csv" without row.names (X column numbered 1 - 650)
```

We can verify whether the file saved correctly.
```{r}
data <- read.csv("allcontentlong.csv") # reads csv file "allcontentlong.csv" to dataframe object called data
str(data) # views internal structure of data
```